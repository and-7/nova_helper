...
## read_data reads in the file
## return_obs_data creates a sorted nested list from the observation data
## combined_sorted_data combines two sets of data and sorts in order of ascending JD
## curve_smoothness returns the 1/nu parameter
## optimize_offset combines two sets of data into one optimally smooth curve
## (minimizing offsets in magnitude between observers)
## combine_obs_data combines all data in a sorted nested list into one optimally smooth curve

## this program will not function unless there is at least one observer code
## with more than 10 observations in a given band
...
def combine_obs_data(obs_data):
    ...
    ## returning the combined data from all obserbers
